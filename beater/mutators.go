...
func (f *MessageFormatter) getChannelMentions() []string{
	...
	for _, c := range f.msg.ChannelMentions {
		channelMentions = append(channelMentions, c.Id)
	}
...
func (f *MessageFormatter) getUserMentions() []string{
	...
	for _, u := range f.msg.UserMentions {
		userMentions = append(userMentions, u.Id)
	}
	return userMentions
}

func (f *MessageFormatter) getContent() []string{
	if f.content != nil { return *f.content }

	var content []string
	content = AppendNonZero(content, f.msg.Content)
	for _, e := range f.msg.Embeds{
		content = AppendNonZero(content, e.Title, e.Description)
		if e.Footer != nil { content = AppendNonZero(content, e.Footer.Text )}
		if e.Author != nil { content = AppendNonZero(content, e.Author.Name )}
		if e.Fields != nil{
...
func (f *MessageFormatter) getHasArray() []int{
	var has []int
	if f.hasAttachments { has = append(has, 2) }
	if f.hasEmbeds { has = append(has, 1) }
	if len(*f.links) > 0 { has = append(has, 0) }
	return has
}
